    ...
    for epoch in range(5000):
                ...
                if (epoch + 1) % 20 == 0:
                    ...
                    if acc > best_acc:
                        ...
                        best_micro = micf1
                        best_macro = macf1
    print(f'Best Epoch: {best_epoch}, Best Accuracy: {best_acc:.6f}, Best F1Mi: {best_micro:.6f}, Best F1Ma: {best_macro:.6f}')


def main():
    from data_preprossing import load_data
    device = torch.device('cuda:2')
    data = load_data('Cora', device)
    ...
    gconv = GConv(input_dim=data.num_features, hidden_dim=32, activation=torch.nn.ReLU, num_layers=2).to(device)
